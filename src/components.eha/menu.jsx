...
import { getMenu } from "../components/headers/menu";
...
export const MenuEha = () => {
    ...
    return <div className="w-full col-span-8 p-2">
        <div className="grid grid-cols-6 gap-3">
            {menu ? menu.ehaMenu?.map((d, k) => {
                let name = d.name.toLowerCase()
                return name !== "scan" ? <NavPage
                    key={k}
                    to={d.url}
                    title={d.name}
                    icons={({ match }) => {
                        return <Icon url={d.url} match={match}></Icon>
                    }}
                    className={({ isActive }) =>
